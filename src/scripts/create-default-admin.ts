  ...
  try {
    ...
    if (Array.isArray(existingSuperAdmins) && existingSuperAdmins.length > 0) {
      ...
      await connection.execute(
        ...
        [hashedPassword, superAdmin.name, superAdmin.phone, superAdmin.email]
      );
      ...
      await connection.execute(
        ...
        [superAdmin.name, superAdmin.email, hashedPassword, superAdmin.phone, 1]
      );
    ...
    let agencies: any[] = [];
    try {
      const [agencyResults] = await connection.execute(
        'SELECT agency_id, name FROM agencies ORDER BY agency_id LIMIT 1'
      ) as [any[], any];
    ...
    if (agencies.length > 0) {
      ...
      const [existingAgencyAdmins] = await connection.execute(
      ...
      if (Array.isArray(existingAgencyAdmins) && existingAgencyAdmins.length > 0) {
        ...
        const salt = await bcrypt.genSalt(10);
        const hashedPassword = await bcrypt.hash(agencyAdmin.password, salt);
        
        await connection.execute(
          'UPDATE admins SET password_hash = ?, name = ?, agency_id = ? WHERE email = ?',
          [hashedPassword, agencyAdmin.name, agencyAdmin.agency_id, agencyAdmin.email]
        );
        ...
        await connection.execute(
          ...
          [agencyAdmin.name, agencyAdmin.email, hashedPassword, agencyAdmin.agency_id]
        );
